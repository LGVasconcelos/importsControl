...
export interface MlSalesSummary {
  ...
  totalFees: number;
  netRevenue: number;
  orders: { id: string; date: string; total: number; fee: number; net: number; debug: { net_received_amount: number; sale_fees: number; shipping_base_cost: number; formula: string }; items: { title: string; quantity: number; unitPrice: number }[] }[];
}
